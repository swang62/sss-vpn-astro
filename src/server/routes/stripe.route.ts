  ...
  .post(
    ...
    async (c) => {
      ...
      const config: Stripe.BillingPortal.SessionCreateParams =
        plan && product && subscriptionId && itemId
          ? {
  ...
  .post("/webhook", async (c) => {
    ...
    switch (event.type) {
      ...
      case "checkout.session.completed": {
        ...
        const isNewSubscription = session.custom_fields?.find(
          (field) => (field.key = "auto_renew")
        );
        ...
        if (session.status === "complete") {
          if (subscriptionId && isNewSubscription) {
            ...
            c.var.logger.debug(
              `Set subscription to auto-renew:${isAutoRenew} for ${stripeCustomerId}`
            );
          }
        }
        break;
