...
function ApiStatus(props: Props) {
  ...
  const getEndpoint = async () => {
    ...
    switch (endpoint) {
      ...
      case "/user":
        ...
        break;
      case "/usage":
        client = apiClient.hiddify.usage.$get;
        break;
      case "/stripe":
        client = apiClient.stripe.user.$get;
        break;
      case "/server-error":
  ...
  const getUser = async () => {
    ...
    const { data, error } = await parseApi(apiClient.user[":id"].$get({ param: { id: userId } }));
    if (!data || error) {
      toast.error(error);
    }
    setCode(JSON.stringify(data || error, null, 2));
  ...
  return (
    <div className="flex flex-col w-full pb-0 gap-4 md:pt-6">
      <div className="flex flex-row justify-start gap-2">
        <Button loading={loading} variant="secondary" onClick={getEndpoint} disabled={disabled} className="w-16">
          Request
        </Button>
      ...
      <div className="flex flex-row justify-start gap-2">
        <Button loading={loading} variant="secondary" onClick={getUser} disabled={disabled} className="w-16">
          Get
        </Button>
